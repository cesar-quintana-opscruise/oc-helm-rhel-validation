...
usage() {
cat << EOF
Usage: bash ops_sanity_check.sh <COMMAND> <SUB_COMMAND>
Available Commands:
...
if [ -f 'opscruise-values.yaml' ]; then
  OC_DOCKER_SECRET_USERNAME=$(grep DOCKER_USERNAME opscruise-values.yaml | awk -F': ' '{print $2}' | sed s/[\"]//g | tr -d '\r')
  OC_DOCKER_SECRET_PASSWORD=$(grep DOCKER_PASSWORD opscruise-values.yaml | awk -F': ' '{print $2}' | sed s/[\"]//g | tr -d '\r')
  OC_DOCKER_SECRET_EMAIL=$(grep DOCKER_EMAIL opscruise-values.yaml | awk -F': ' '{print $2}' | sed s/[\"]//g | tr -d '\r')
else
...
spec:
  containers:
  - name: net-util
    image: opscruiseindia/oc-net-utils:v1.3
...
post-check() {
  ...
  for ns in "${OC_SOUTH_NAMESPACES[@]}"; do
    ...
    for pod in ${out[@]}; do
      ...
      len=$(echo "$stat" | wc -w)
      if [ "$len" -eq 0 ]; then
        con_stat=$(kubectl get pod "${pod}" -n "${ns}" -o jsonpath='{.status.phase}')
        con_name="NA"
        con_rsn=$(kubectl get pod "${pod}" -n "${ns}" -o jsonpath='{.status.conditions[*].message}')
        data="$pod,$con_name,$con_rsn"
        not_running+=("$(echo -e "$data")")
      elif [[ "$stat" == *"false"* && "$len" -gt 0 ]]; then
        for (( i=0; i<len; i++ ));
        do
  ...
  if [ -n "$ig" ]; then
    ...
    for i in $ig; do
      if [[ "$i" == *"azuregw"* ]]; then
        ...
        if kubectl logs "$i" -n opscruise | grep -wi Connected; then
        ...
        if [ -f 'opscruise-values.yaml' ]; then
          AWS_REGION=$(grep region opscruise-values.yaml | head -1 | awk -F': ' '{print $2}' | sed s/[\"]//g | tr -d '\r')
          AWS_ACCESS_KEY_ID=$(grep aws_access_key_id opscruise-values.yaml | head -1 | awk -F': ' '{print $2}' | sed s/[\"]//g | tr -d '\r')
          AWS_SECRET_ACCESS_KEY=$(grep aws_secret_access_key opscruise-values.yaml | head -1 | awk -F': ' '{print $2}' | sed s/[\"]//g | tr -d '\r')
          export AWS_REGION
...
EOF
    ...
    core_check=$(kubectl describe pod "${ne}" -n opscruise | grep Image:)
    if [[ "$core_check" == *"core"* ]]; then
      echo -e "[ ${BGREEN}OK${NC} ] ... BTF enabled Node-exporter. Kernel headers not mandatory"
    else
  ...
  unset jq_parsed
  prom_name=$(kubectl get pods -n collectors -o wide | grep prometheus | awk '{print $1}')
  prom_ip=$(kubectl get pod -n collectors "${prom_name}" -o jsonpath='{.status.podIP}')
  kubectl exec -it "${oc_pod_name}" -n "${OC_NAMESPACE}" -- /bin/sh -c "curl -s http://${prom_ip}:9090/api/v1/targets?state=active > jq.txt"
  jq_parsed=$(kubectl exec -it "${oc_pod_name}" -n "${OC_NAMESPACE}" -- /bin/sh -c "cat jq.txt | jq -r '.data.activeTargets[] | \"\(.discoveredLabels.__meta_kubernetes_service_name // .discoveredLabels.__meta_kubernetes_pod_label_opscruiseProduct),\(.labels.instance),\(.discoveredLabels.__metrics_path__),\(.health)\"' | sort | tr -d '\r'")
  ...
  do
    svc_name=$(echo "${jqp}" | awk -F',' '{print $1}' | tr -d '\r')
    prome_tgt=$(echo "${jqp}" | awk -F',' '{print $2}' | tr -d '\r')
    mpath=$(echo "${jqp}" | awk -F',' '{print $3}' | tr -d '\r')
    health=$(echo "${jqp}" | awk -F',' '{print $4}' | tr -d '\r')
    if ! [[ "$svc_name" =~ ^(k8sGW|loggw-loki|promGW|awsGW|azuregw|gcpgw|promgw-service)$ ]]; then
